					...
					<div class="userCenterBox boxCenterList clearfix" style="_height: 1%;">
						...
						<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
							<tr>
								...
								<td align="center" bgcolor="#ffffff">房源</td>
							...
							<c:forEach items="${ordersList}" var="orders">
								<tr>
									...
									<td align="center" bgcolor="#ffffff"><c:if test="${orders.status eq '未付款'}">
										...
										</c:if> <c:if test="${orders.status eq '已接受'}">
											<a href="index/over.action?id=${orders.ordersid}">确认约见</a>
										</c:if> <c:if test="${orders.status eq '已约见'}">
											<a href="index/preTopic.action?id=${orders.ordersid}">评价房源</a>
										</c:if></td>
								</tr>
							</c:forEach>
						</table>
